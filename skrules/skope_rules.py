...
class SkopeRules(BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        rules_ = []
        for estimator, samples, features in tqdm(zip(self.estimators_,
                                                self.estimators_samples_,
                                                self.estimators_features_), total=len(self.estimators_), desc='\tRule Extracting ...'):
        ...
        # self.rules_ = sorted(self.rules_, key=lambda x: -x[1][0])
