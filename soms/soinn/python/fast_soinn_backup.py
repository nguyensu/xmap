...
def learning(input_data=None, max_nepoch=None, spread_factor=0.1, lamb=None):
    ...
    age_max = sample_size
    for ep in range(max_nepoch):
        # print("Epoch #", ep)
        np.random.shuffle(data)
        total_newnodes = 0
        n_newnodes = 0
        for i in trange(2, sample_size, desc='SOINN - EPOCH ' + str(ep+1) + "/" + str(max_nepoch), leave=True):
            ...
            if value[0] > threshold[index[0]] or value[1] > threshold[index[1]]:
                ...
                if nodes_class_id[index[0]] == 0 or nodes_class_id[index[1]] == 0:
                    ...
                    if winner_condition or runner_condition: # winner or runner have high density to be treated as important node and maintain their connections
                        ...
                        nodes_class_id[nodes_class_as_winner] = winner_runner_combine_class #winner and runner should be in the same class
            ...
            # print(i, "/", sample_size)
            if (i + 1) % lamb == 0 or ((i == sample_size-1) and (ep == max_nepoch - 1)): # or :
                # mean point
                ...
                q_density = np.quantile(nodes_density, math.pow(1 - spread_factor, 4))
                ...
                age_max = (1+spread_factor)*n_newnodes
